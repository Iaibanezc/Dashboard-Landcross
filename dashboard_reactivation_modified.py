...
with st.sidebar:
    ...
    extra_ids = st.multiselect(
        ...
        format_func=lambda x: f"DT {int(x)}" if pd.notna(x) else str(x),
    )
...
with tab_truck:
    ...
    st.markdown(
        f"""
        <div class="kpi-grid">
          <div class="kpi-card"><div class="kpi-label">Selected truck</div><div class="kpi-value">DT {int(selected_truck)}</div><div class="kpi-sub">Individual analysis</div></div>
