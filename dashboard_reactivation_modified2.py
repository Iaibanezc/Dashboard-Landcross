...
# Permanent filter: 19 trucks with lowest Weighted criteria, tie-broken by cost
...
with st.sidebar:
    ...
    st.markdown(
        ...
        "Thresholds only affect the component count charts — "
        "kit flags and fleet costs come directly from the Excel file.</p>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        "The core filter keeps the 19 trucks with the lowest Weighted Criteria. "
    ...
    <div class="lc-header">
      <div>
        ...
        <div class="lc-header-subtitle">
          ...
          ({len(TOP19_DTS)} core filter + {len(extra_dts)} additional)
        </div>
      </div>
...
with tab_gantt:
    ...
    def labour_color(v):
        # v in [0,1]: 0 = shortest = lighter, 1 = longest = darker
        r = int(255)
        g = int(107 - 70 * v)
        b = int(0)
...
st.markdown(
    ...
    '<span>Data: Data_base_Reactivation.xlsx</span>'
    '</div>',
    unsafe_allow_html=True,
)
